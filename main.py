        ...
        try:
            ...
            try:
                ...
                with urllib.request.urlopen(
                    f"https://ipinfo.io/{ip}/json", timeout=3
                ) as response:
...
def dns():
    while True:
        ...
        try:
            ...
            try:
                cadResult = subprocess.run(
                    ["nslookup", "-type=CAA", target],
                    capture_output=True,
                    text=True,
                    timeout=5,
                )
        ...
        if "/" in host and host.count(".") == 3:
            try:
                ...
                with ThreadPoolExecutor(max_workers=50) as executor:
                    if customPort:
                        results = list(
                            executor.map(lambda h: pingHost(str(h), customPort), hosts)
                        )
                    else:
                        ...
                        if result.returncode == 0:
                            ...
                            for line in lines:
                                if "time=" in line:
                                    timeMatch = line.split("time=")[1].split("ms")[0]
                                    print(
                                        f"\033[38;5;147m{host} | online | {timeMatch}ms | icmp\033[0m"
                                    )
                                    break
                        else:
                    ...
                    while True:
                        ...
                        try:
                            ...
                            if result == 0:
                                responseTime = (endTime - startTime) * 1000
                                print(
                                    f"\033[38;5;147m{host} | online | {responseTime:.1f}ms | {port}\033[0m"
                                )
                            else:
